  ...
  return (
    <Card className="bg-white border-slate-150 shadow-sm">
      ...
      <CardContent className="p-6 pt-4 space-y-3.5">
        <div>
          <span className="block text-[10px] uppercase font-bold text-slate-700">Subject Expertise</span>
        ...
        <div>
          <span className="block text-[10px] uppercase font-bold text-slate-700">Experience</span>
          <span className="text-sm font-semibold text-slate-900 mt-0.5 block">{tutor.experience}</span>
        </div>
        <div>
          <span className="block text-[10px] uppercase font-bold text-slate-700">Availability</span>
          <span className="text-sm font-semibold text-slate-900 flex items-center gap-1.5 mt-0.5">
        ...
        {tutor.subjectEntries && tutor.subjectEntries.length > 0 && (
          <div>
            <span className="block text-[10px] uppercase font-bold text-slate-700 mb-1.5">Syllabus Coverage</span>
            <div className="flex flex-wrap gap-1.5">
              {tutor.subjectEntries.map((entry, i) => (
                <div key={i} className="text-[10px] font-semibold text-slate-800 bg-slate-100 border border-slate-200 rounded-md px-2 py-1">
