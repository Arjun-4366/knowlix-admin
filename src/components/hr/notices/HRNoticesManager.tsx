...
function NoticeFormModal({
  ...
  const [category, setCategory] = useState<HRNoticeCategory>(initial?.category ?? "notice");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ title, content, category, priority, department: department || undefined, audience });
  };
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl border border-slate-150 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-slate-800 text-base">{isEdit ? "Edit Notice" : "Create Notice"}</h3>
              <p className="text-[10px] text-slate-600 font-semibold mt-0.5">
                {isEdit ? "Update the notice details below" : "Fill in the details to publish a notice"}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase font-bold text-slate-600">Category</Label>
              <Select value={category} onValueChange={(v) => setCategory(v as HRNoticeCategory)}>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={priority} onValueChange={(v) => setPriority(v as HRNoticePriority)}>
                <SelectTrigger className="h-9 text-sm font-semibold bg-slate-50 border-slate-200">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase font-bold text-slate-600">Audience</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase font-bold text-slate-600">Department</Label>
          ...
          <div className="space-y-1.5">
            <Label className="text-[10px] uppercase font-bold text-slate-600">Title *</Label>
            <Input
              value={title}
          ...
          <div className="space-y-1.5">
            <Label className="text-[10px] uppercase font-bold text-slate-600">Content *</Label>
            <Textarea
              value={content}
  ...
  return (
    <div className="rounded-2xl border border-slate-150 bg-white shadow-sm hover:shadow-md transition-shadow p-5 space-y-3">
      <div className="flex items-start gap-3">
        <div className={cn("w-9 h-9 rounded-xl border flex items-center justify-center flex-shrink-0", meta.bg)}>
          <Icon className={cn("w-4 h-4", meta.color)} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-slate-800 leading-tight line-clamp-2">{notice.title}</p>
          <p className="text-[11px] text-slate-600 mt-1">
            {format(parseISO(notice.createdAt), "dd MMM yyyy, hh:mm a")}
            {notice.department ? ` · ${notice.department}` : ""}
            {notice.audience && notice.audience !== "all" ? ` · For ${notice.audience}` : ""}
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <Badge variant="outline" className={cn("rounded-full text-[10px] font-bold px-2.5 py-0.5 border", meta.bg, meta.color)}>
          {meta.label}
        </Badge>
        <Badge variant="outline" className={cn("rounded-full text-[10px] font-bold px-2.5 py-0.5 border", PRIORITY_COLOR[notice.priority])}>
...
export default function HRNoticesManager() {
  ...
  useEffect(() => { setPage(1); }, [categoryFilter, priorityFilter, audienceFilter]);

  const { data: noticesRes, isLoading } = useGetHRNotices({
    search: debouncedSearch || undefined,
    category: categoryFilter !== "all" ? (categoryFilter as HRNoticeCategory) : undefined,
    priority: priorityFilter !== "all" ? (priorityFilter as HRNoticePriority) : undefined,
  ...
  const notices: IHRNotice[] = (noticesRes?.data ?? []) as IHRNotice[];
  const summary = noticesRes?.summary;
  const total = noticesRes?.total ?? 0;
  const totalPages = noticesRes?.totalPages ?? 1;

  const hasFilters = debouncedSearch || categoryFilter !== "all" || priorityFilter !== "all" || audienceFilter !== "all";

  const handleCreate = (data: ICreateHRNoticePayload) => {
    createNoticeMutation.mutate(data, { onSuccess: () => setShowModal(false) });
  };
  ...
  const handleUpdate = (data: ICreateHRNoticePayload) => {
    ...
    updateNoticeMutation.mutate(
      ...
      { onSuccess: () => { setShowModal(false); setEditTarget(null); } },
    );
  };
  ...
  const handleDelete = (id: string) => {
    confirm({
      ...
      confirmText: "Delete",
      variant: "danger",
      onConfirm: async () => { deleteNoticeMutation.mutate(id); },
    });
  };

  const openCreate = () => { setEditTarget(null); setShowModal(true); };
  const openEdit = (notice: IHRNotice) => { setEditTarget(notice); setShowModal(true); };
  ...
  return (
    <div className="space-y-6 w-full pb-10">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Announcements"
          value={summary?.category.announcement ?? 0}
          icon={Megaphone}
          color="bg-sky-100 text-sky-600"
        />
        <StatCard
          label="Notices"
          value={summary?.category.notice ?? 0}
          icon={Bell}
          color="bg-amber-100 text-amber-600"
        />
        <StatCard
          label="High Priority"
          value={summary?.priority.high ?? 0}
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-[200px] max-w-full">
            ...
            <Input
              ...
              className="w-full pl-8 pr-3 h-9 text-xs rounded-xl border border-slate-200 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-200 font-medium text-slate-700 placeholder:text-slate-400"
            />
          </div>

          {/* Category */}
          <Select value={categoryFilter} onValueChange={(v) => setCategoryFilter(v as HRNoticeCategory | "all")}>
            <SelectTrigger className="w-[145px] h-9 text-xs font-semibold bg-slate-50 border-slate-200 rounded-xl">
          ...
          <Select value={audienceFilter} onValueChange={setAudienceFilter}>
            ...
            <SelectContent>
              {AUDIENCES.map((a) => (
                <SelectItem key={a} value={a} className="text-xs">{a.charAt(0).toUpperCase() + a.slice(1)}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          {hasFilters && (
            <button
              ...
              className="text-[10px] font-semibold text-slate-600 hover:text-slate-800 flex items-center gap-1 transition-colors"
            >
      ...
      {isLoading ? (
        ...
        <Card className="rounded-2xl border-slate-150 p-10 text-center bg-white shadow-sm">
          ...
          <p className="text-xs text-slate-600 mt-1">
            {hasFilters ? "Try adjusting your filters or search term." : "Publish your first notice using the button above."}
          </p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4">
          {notices.map((notice) => (
            <NoticeCard key={notice.id} notice={notice} onEdit={openEdit} onDelete={handleDelete} />
          ))}
        </div>
      )}
      ...
      {total > PAGE_LIMIT && (
        <div className="flex items-center justify-between px-1">
          <p className="text-[11px] text-slate-600 font-medium">
            Showing {notices.length === 0 ? 0 : (page - 1) * PAGE_LIMIT + 1}–{Math.min(page * PAGE_LIMIT, total)} of {total} notices
          </p>
          <div className="flex items-center gap-1.5">
            <Button
              ...
              disabled={page <= 1}
              className="h-8 w-8 rounded-xl border-slate-200 text-slate-600 disabled:opacity-40"
            >
